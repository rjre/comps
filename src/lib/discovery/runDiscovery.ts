...
async function processListingItem(item: ListingItem, source: FeedSource): Promise<ItemResult> {
  ...
  // Shared giveaway platforms (DMRI, Gleam.io, KingSumo) get the adapter
  // that actually works on them rather than the generic form-filler —
  // see registry.ts's detectAdapterKey.
  const adapterKey = detectAdapterKey(entryUrl);
