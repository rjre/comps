...
/**
 * Village People Magazine — "WIN an Ultimate Ears BOOM 4 Portable Speaker"
 * (with John Lewis Norwich), on village-people.info/enter-our-competition/.
 * Same site/page as village-people-banham-zoo, but a genuinely separate
 * Contact Form 7 instance (id 105434) — the page hosts several unrelated
 * competitions, each with its own <form data-wpcf7-id="...">. Identical
 * field shape to the Banham Zoo form (name / single address textarea /
 * phone / email), no marketing checkbox on the form itself.
 *
 * This site's CF7 install runs reCAPTCHA v3 and — per villagePeopleBanhamZoo.ts's
 * own root-caused finding — consistently score-rejects submissions as
 * spam regardless of a valid-looking token, across many attempts over
 * many hours. That diagnosis was made against form 103083 specifically,
 * but the reCAPTCHA v3 site key/threshold is a per-site (not per-form)
 * WordPress plugin setting, so the same rejection is expected here too;
 * built anyway (never solved/evaded) so the outcome is confirmed and
 * logged rather than assumed, same "submit-time reCAPTCHA, not a
 * page-load block" category as c2c-blowout-company.
 */
export const villagePeopleUltimateEarsAdapter: CompetitionAdapter = {
  key: "village-people-ultimate-ears",
  ...
  async enterCompetition({ page, competitionUrl, profile, log, dryRun }: AdapterContext): Promise<EntryOutcome> {
    ...
    const form = page.locator('form:has(input[name="_wpcf7"][value="105434"])');
    if ((await form.count()) === 0) {
      await log.warn("Ultimate Ears BOOM 4 entry form (wpcf7 id 105434) not found — page may have changed");
    ...
    // form's own endpoint, id 105434) — the displayed message is generic,
    // so the raw JSON `status` field is read directly instead.
    const [ajaxResponse] = await Promise.all([
      page
        .waitForResponse((r) => r.url().includes("/contact-forms/105434/feedback") && r.request().method() === "POST", { timeout: 15000 })
        .catch(() => null),
      submit.click(),
    ]);
