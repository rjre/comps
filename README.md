...
- **A possible win** — recorded to `/wins` and a notification sent.
  ...
  somewhere nobody is looking. With no `WIN_NOTIFY_EMAIL` set, wins are
  never archived.
...
Set `WIN_NOTIFY_EMAIL` to get win alerts. It sends through the same Gmail
account being scanned, so it needs no extra credential — but does need the
